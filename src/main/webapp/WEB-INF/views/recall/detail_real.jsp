        ...
        <body>
            ...
            <script>
                ...
                function handleButtonClicked(index) {
                    ...
                    if (recallButtons.length > index) {
                        recallButtons[index].parentNode.removeChild(recallButtons[index]);
                    }
                }

                // 리콜 버튼 클릭 이벤트 설정
                var recallButtons = document.querySelectorAll('.recall-button');
                for (var i = 0; i < recallButtons.length; i++) {
                    recallButtons[i].addEventListener('click', function (event) {
                        var buttonIndex = Array.from(recallButtons).indexOf(event.target);
